...
main()
{
	...
	for (int i=0; i<state.noJunctionBoxes; i++)
		circuitLengths[i] = getCircuitLength(state.circuits[i]);

	qsort(circuitLengths, state.noJunctionBoxes, sizeof(int), intCmp);

	uint64_t out = 1;
	for (int i=0; i<3; i++)
		out *= circuitLengths[state.noJunctionBoxes - i - 1];

	printf("The lengths of the three longest circuits multiplied: %lu\n", out);
